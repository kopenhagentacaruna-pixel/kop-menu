(function (global) {
  ...
  function parseLooseRow(rowText, previousText = "") {
    const prices = allMatches(PRICE_RE, rowText);
    if (!prices.length) return null;

    const price = prices[prices.length - 1][1];
    const rebuilt = descriptionUntilLastWeight(rowText);
    const beforePrice = stripLeftColumns(rowText.slice(0, prices[prices.length - 1].index));
    let source = rebuilt || beforePrice || previousText;
    if (!source || /subtotal|total|relatorio|página|pagina|descri/i.test(asciiKey(source))) return null;

    let weight = "";
    source = cleanName(source);
    [source, weight] = extractWeight(source);
    [source, weight] = repairDescriptionTail(source, weight, rowText);

    if (!source || source.length < 3 || /subtotal|total|relatorio|pagina|descri/.test(asciiKey(source))) return null;
    return { name: source, weight, price: normalizePrice(price) };
  }

  function parseLooseFragments(fragments) {
    const rows = groupRows(fragments, 4.5);
    const items = [];
    let previousText = "";

    for (const row of rows) {
      const rowText = row.fragments.map((fragment) => fragment.text).join(" ").replace(/\s+/g, " ").trim();
      if (!rowText) continue;

      const item = parseLooseRow(rowText, previousText);
      if (item) {
        items.push(item);
        previousText = "";
        continue;
      ...
      if (!PRICE_RE.test(rowText) && cleanName(rowText).length >= 3) {
  ...
  async function extractWithPdfJs(arrayBuffer) {
    ...
    if (pdfjs.GlobalWorkerOptions && typeof document !== "undefined") {
      pdfjs.GlobalWorkerOptions.workerSrc = new URL("./pdf.worker.mjs", pdfjsUrl).href;
    }
    ...
    for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
      ...
      const positionedItems = parsePositionedFragments(fragments);
      items.push(...(positionedItems.length ? positionedItems : parseLooseFragments(fragments)));
    }
  ...
  async function extractRelatorio5301FromArrayBuffer(arrayBuffer) {
    const diagnostics = [];
    global.lastRelatorio5301Debug = "";

    try {
      const pdfJsItems = await extractWithPdfJs(arrayBuffer);
      diagnostics.push(`PDF.js encontrou ${pdfJsItems.length} produto(s).`);
      if (pdfJsItems.length) {
        global.lastRelatorio5301Debug = diagnostics.join(" ");
        return pdfJsItems;
      }
    } catch (error) {
      diagnostics.push(`PDF.js falhou: ${error.message || error}.`);
    }

    try {
      const builtInItems = await extractWithBuiltInReader(arrayBuffer);
      diagnostics.push(`Leitor alternativo encontrou ${builtInItems.length} produto(s).`);
      global.lastRelatorio5301Debug = diagnostics.join(" ");
      return builtInItems;
    } catch (error) {
      diagnostics.push(`Leitor alternativo falhou: ${error.message || error}.`);
      global.lastRelatorio5301Debug = diagnostics.join(" ");
      return [];
    }
  }
